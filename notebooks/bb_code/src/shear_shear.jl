...
function heath_integral(cosmo, z)
    ...
    return bb.compute_adimensional_hubble_factor(z, cosmo) * integral_val
end
...
function shear_prefactor(
    ...
    #com_dist = bb.compute_χ.(z_range, Ref(cosmo));
    ...
    #Hubble function
    H_array = bb.compute_hubble_factor.(z_range, Ref(cosmo))
    #plot
    ...
    plot(z_range, prefac_shear,
         ...
         legend=:topright, size=(1400, 800), titlefontsize=15,
         labelfontsize=15, legendfontsize=7, dpi=300)
...
function shear_prefactor_cheb(
    ...
    z_cheb_nodes = bb.get_clencurt_grid_z(zmin, zmax, n_cheb)
...
function compute_prefactor_chebcoeffs(W_vals::AbstractVector; output_dir::AbstractString)
    plan = bb.plan_fft(W_vals)
    cheb_coeff = bb.fast_chebcoefs(W_vals, plan)
