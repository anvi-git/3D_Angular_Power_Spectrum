...
function galaxy_prefactor(
    ...
    Main.append_to_log(output_dir, "Processing galaxy prefactor...")
    ...
    Main.append_to_log(output_dir, "Done. Moving on to the growth factor...")
    ######
    # GROWTH FACTOR
    ...
    Main.append_to_log(output_dir, "Done. Moving on to the Hubble factor...")
    ######
    # Hubble factor
    ...
    Main.append_to_log(output_dir, "Done. Moving on to the number density...")
    ######
    #n(z)  
    ...
    plot(z, nz, label=L"n(z)", xlabel=L"z", 
         ylabel=L"n(z)", legend = :topleft; plot_style...)
    savefig(joinpath(output_dir, "plots/kernels/nz.png"))
    println("n(z) vs redshift plot saved.")
    Main.append_to_log(output_dir, "Both non normalized...")
    plot(z, n_chi_norm, label=L"n(z) normalized", xlabel=L"z", 
          ylabel=L"n(z) normalized", legend = :topleft; plot_style...)
    ...
    println("n(z) vs redshift plot saved.")
    Main.append_to_log(output_dir, "and normalized...")
    ...
    Main.append_to_log(output_dir, "Done. Moving on to the complete prefactor W(χ)...")
    ######
    # W(z)
    ...
    plot(z, 
          ...
          title=L"Normalized b(z)"; plot_style...)
        plot!(z, 
                x ./ maximum(x), 
                label=L"\chi(z)",
    ...
    Main.append_to_log(output_dir, "Done.")
...
function galaxy_prefactor_cheb(xmin::Number, 
    ...
    Main.append_to_log(output_dir, "redshift computed.")
    ...
    Main.append_to_log(output_dir, "bias computed.")
    ...
    Main.append_to_log(output_dir, "growth computed.")
    ...
    Main.append_to_log(output_dir, "normalization computed.")
    ...
    Main.append_to_log(output_dir, "Hubble factor computed.")
    W_vals = zeros(length(chi_cheb_nodes))
    W_vals = chi_cheb_nodes.^2 .* b_cheb .* D_cheb .* nz_cheb .* Hubble_cheb
    Main.append_to_log(output_dir, "size of the interpolated prefactor array is $(length(W_vals)).")
    Main.append_to_log(output_dir, "Prefactor computed.")
    ...
    Main.append_to_log(output_dir, "The galaxy_prefactor_cheb function has been completed.")
    return W_vals
end

function compute_prefactor_chebcoeffs(W_vals::AbstractVector; output_dir::AbstractString)
    plan = Blast.plan_fft(W_vals)
    Main.append_to_log(output_dir, "size of the plan wave is $(length(plan)).")
    cheb_coeff = Blast.fast_chebcoefs(W_vals, plan)
    Main.append_to_log(output_dir, "size of the Chebyshev coefficients is $(length(cheb_coeff)).")
