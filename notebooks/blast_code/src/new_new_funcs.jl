...
function W_tilde_computed(ℓ::Number, zmin::Number, zmax::Number, kmin::Number, kmax::Number,
    ...
    T, Bessel1 = bessel_func(ℓ, zmin, zmax, kmin, kmax, z_range, n_cheb, N, chi_of_z)
    ...
    Threads.@threads for j in 1:Nkp
        kj = kp[j]
        for i in 1:N
            @inbounds Bessel2[j, i] = sphericalbesselj(ℓ, chi_vals[i] * kj)
        end
    end
    ...
    for ic in 1:(n_cheb + 1) 
        @tturbo for ik in 1:Nk, ikp in 1:Nkp
            Cij = zero(eltype(w))
            for iN in 1:N
                Cij += T[ic, iN] * Bessel1[ik, iN] * Bessel2[ikp, iN] * w[iN]
            end
